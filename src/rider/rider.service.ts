...
export class RiderService {
  ...
  async createRider(createRiderDto: any): Promise<Rider> {
    ...
    const existingRider = await this.riderModel.findOne({ phone: createRiderDto.phone });
    if (existingRider) {
      throw new ConflictException('User already registered as a rider');
    }
    
    // Map user ID to rider and create entry
    const newRider = new this.riderModel({ ...createRiderDto, userId: user._id });
